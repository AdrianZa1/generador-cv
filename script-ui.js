...
function improveEntryDesc(id) {
  ...
  showQuestionModal({ title: 'Mejorar descripción', intro: 'Responder 5 preguntas para mejorar la descripción.', questions }, (answers) => {
...
async function callOpenAI(systemPrompt, userPrompt) {
  ...
  try {
    ...
    const r = await fetch('/api/gpt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, max_tokens: 400 }) });
    if (r.ok) {
...
function generateTextFromAnswers(context, answers, meta) {
  // context: 'profile' or 'exp'
  // meta: for exp -> {company, role}
  let text = '';
  if (context === 'profile') {
    // answers: [specialty, years, skills, objective, differentiator]
    ...
    // answers: [responsibility, achievement, tools, impact, summary]
...
// Palette selector for template-2 (modern)
...
function showPage(n) {
  ...
  if (n === 2) {
    // Inline validation: show error messages under each field
...
// createBioInteractive implemented earlier; placeholder removed.